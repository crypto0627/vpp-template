    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {onSelectDate && (
              <span className="text-xs text-white/40">點擊日期查看逐時數據</span>
            )}
            <span className="text-sm text-white/50">{dailyReport.length} 天</span>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-0">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#3A2415] bg-[#1E1208]">
                <th className="text-left py-2.5 px-3 text-white/60 font-semibold">
                  日期
                </th>
                <th className="text-right py-2.5 px-3 text-white/60 font-semibold">
                  小時
                </th>
                <th
            ...
            <tbody>
              {display.map((d) => (
                <tr
                  key={d.date}
                  className={`border-b border-[#3A2415]/50 ${
                    selectedDate === d.date
                      ? "bg-[#E8883E]/15 border-2 border-[#E8883E] hover:bg-[#E8883E]/20"
                      : "hover:bg-[#1E1208]"
                  }`}
                >
            ...
            <tfoot>
              <tr className="border-t-2 border-[#3A2415] bg-[#1E1208] font-semibold text-sm">
                <td className="py-2.5 px-3 text-white/80">合計</td>
        ...
        {dailyReport.length > 10 && (
          <div className="px-4 py-3 border-t border-[#3A2415]/50 text-center">
            <button
